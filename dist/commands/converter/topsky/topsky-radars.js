...
exports.parseTopSkyRadars = parseTopSkyRadars;
exports.mergeRawVideo = mergeRawVideo;
...
function radar(block, result) {
    const name = (block.header.fields[1] ?? "").trim();
    if (!name) {
        (0, topsky_lexer_1.warn)(block.header, "RADAR block has no station name");
        return null;
    }
    let location = null;
    const rawVideo = {
    ...
    for (const line of block.lines) {
        switch (line.keyword) {
            case "POSITIONS":
                ...
                break;
            case "LOCATION":
                location = (0, topsky_coords_1.parseLatLon)(line.fields[1], line.fields[2]);
                if (!location)
                    (0, topsky_lexer_1.warn)(line, `station "${name}" has an unreadable LOCATION`);
                break;
            case "ALTITUDE": {
                const altitude = (0, topsky_lexer_1.numberField)(line, 1, "antenna altitude");
                if (altitude !== undefined)
                    rawVideo.antennaAltitudeFt = altitude;
                break;
            }
            case "BEAMWIDTH": {
                const beamwidth = (0, topsky_lexer_1.numberField)(line, 1, "beamwidth");
                if (beamwidth !== undefined && beamwidth > 0)
                    rawVideo.beamwidthDeg = beamwidth;
                break;
            }
            case "PULSEWIDTH": {
                const pulse = (0, topsky_lexer_1.numberField)(line, 1, "pulse width");
                if (pulse !== undefined && pulse > 0)
                    rawVideo.pulseWidthUs = pulse;
                break;
            }
            case "MAXANGLE": {
                const angle = (0, topsky_lexer_1.numberField)(line, 1, "maximum elevation angle");
                if (angle !== undefined && angle > 0)
                    rawVideo.maxAngleDeg = angle;
                break;
            }
            case "RANGE": {
                const min = (0, topsky_lexer_1.numberField)(line, 1, "minimum range");
                const max = (0, topsky_lexer_1.numberField)(line, 2, "maximum range");
                if (min !== undefined && min > 0)
                    rawVideo.minRangeNm = min;
                if (max !== undefined && max > 0)
                    rawVideo.maxRangeNm = max;
                break;
            }
            case "CEILING": {
                const ceiling = (0, topsky_lexer_1.numberField)(line, 1, "detection ceiling");
                if (ceiling !== undefined && ceiling > 0)
                    rawVideo.ceilingFt = ceiling;
                break;
            }
            case "NOTERRAIN":
            case "TERRAIN":
                ...
                break;
            default:
                (0, topsky_lexer_1.warn)(line, `unknown RADAR property "${line.keyword}"`);
                break;
        }
    }
    if (!location) {
        (0, topsky_lexer_1.warn)(block.header, `station "${name}" has no LOCATION and cannot be matched to a radars.json station`);
        return null;
    }
    if (rawVideo.positions.length === 0) {
        (0, topsky_lexer_1.warn)(block.header, `station "${name}" has no POSITIONS, so no login callsign will ever select it`);
    }
    return { name, location, rawVideo, at: block.header };
}
function parseTopSkyRadars(file, content) {
    const result = {
        parsed: [],
        matchedByName: 0,
        matchedByLocation: 0,
    ...
    const grouped = (0, topsky_lexer_1.blocks)(lines, ["RADAR"]);
    for (const line of grouped.preamble) {
        (0, topsky_lexer_1.warn)(line, `line before the first RADAR block is dropped ("${line.keyword}")`);
    }
    const seen = [];
    for (const block of grouped.blocks) {
        ...
        if (twin) {
            (0, topsky_lexer_1.warn)(block.header, `station "${parsed.name}" shares a LOCATION with "${twin.name}"; one of the two coordinates is wrong`);
        }
        seen.push(parsed);
 ...
 */
function mergeRawVideo(section, parsed) {
    for (const station of parsed.parsed) {
        const byName = section.stations.find((other) => other.name.trim().toLowerCase() === station.name.trim().toLowerCase());
        if (byName) {
            byName.rawVideo = station.rawVideo;
            parsed.matchedByName++;
            continue;
        }
        const byLocation = section.stations.find((other) => metresBetween({ lat: other.latitude, lon: other.longitude }, station.location) < MATCH_DISTANCE_M);
        if (byLocation) {
        ...
        (0, topsky_lexer_1.warn)(station.at, `station "${station.name}" matches no radars.json station by name or within ${MATCH_DISTANCE_M} m of ` +
...
//# sourceMappingURL=topsky-radars.js.map