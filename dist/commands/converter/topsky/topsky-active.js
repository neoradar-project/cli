...
exports.foldSchedules = foldSchedules;
...
function isDatePeriod(field) {
    return /^\d{4}$/.test(field ?? "") || /^\d{6}$/.test(field ?? "");
}
function timeOfDay(field) {
    const text = (field ?? "").trim();
    if (!/^\d{4}$/.test(text))
        return null;
...
function parseActive(line) {
    const fields = line.fields;
    const first = (fields[1] ?? "").trim().toUpperCase();
    if (fields.length === 2 && first === "1")
        return { kind: "always" };
    if (first === "AUP") {
        const ref = (fields[2] ?? "").trim();
        if (!ref) {
            (0, topsky_lexer_1.warn)(line, "ACTIVE:AUP names no area");
            return null;
        }
        return { kind: "aup", ref };
    }
    if (first === "NOTAM") {
        ...
        if (!icao || !text) {
            (0, topsky_lexer_1.warn)(line, "ACTIVE:NOTAM needs an ICAO and the text to look for");
            return null;
        }
    ...
    if (first === "RWY") {
        ...
        if (!rule) {
            (0, topsky_lexer_1.warn)(line, "ACTIVE:RWY needs ARR and DEP lists");
            return null;
        }
        return rule;
    }
    ...
    if (isDatePeriod(fields[1])) {
        ...
        if (!isDatePeriod(fields[2]) || !fields[3] || !start || !end) {
            (0, topsky_lexer_1.warn)(line, "ACTIVE schedule needs start:end:days:HHMM:HHMM");
            return null;
        }
        const rule = {
            kind: "schedule",
        ...
        if (fields[6] && Number.isFinite(lower))
            rule.lowerFt = lower;
        if (fields[7] && Number.isFinite(upper))
            rule.upperFt = upper;
        const userText = fields.slice(8).join(":").trim();
        if (userText)
            rule.userText = userText;
        return rule;
    }
    (0, topsky_lexer_1.warn)(line, "ACTIVE line matches no known form and the area would never activate");
    return null;
}
 ...
 */
function foldSchedules(rules, zone) {
    const schedules = rules.filter((rule) => rule.kind === "schedule" && rule.zone === "UTC" && rule.from.length === 4);
    ...
    for (const rule of schedules) {
        ...
        if (bucket)
            bucket.push({ rule, local });
        else
            buckets.set(key, [{ rule, local }]);
    }
    const folded = [];
    ...
    for (const bucket of buckets.values()) {
        ...
        for (const entry of sorted)
            consumed.add(entry.rule);
    }
    const kept = rules.filter((rule) => !consumed.has(rule));
    const partial = kept.filter((rule) => rule.kind === "schedule" && rule.zone === "UTC" && rule.from.length === 4).length;
    return { rules: [...kept, ...folded], folded: folded.length, partial };
}
//# sourceMappingURL=topsky-active.js.map