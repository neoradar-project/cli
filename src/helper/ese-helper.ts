...
export class EseHelper {
    ...
    static async parseEseContent(
        eseFilePath: string,
        allNavaids: NseNavaid[],
        isGNG: boolean = false
    ): Promise<ParsedEseContent> {
    ...
    private static finalizeSector(context: SectorHandlerContext): void {
        if (context.currentSector.borders.length === 0) {
            logESEParsingWarning(
                `Sector "${context.currentSector.name}" has no borders defined`
            );
        }
    ...
    private static handleSectorLine(
        line: string,
        result: ParsedEseContent,
        context: SectorHandlerContext,
        allNavaids: NseNavaid[]
    ...
    private static handleCircleSectorLine(
        line: string,
        context: SectorHandlerContext,
        allNavaids: NseNavaid[]
    ): void {
        ...
        if (!geo || !this.isValidGeoCoord(geo)) {
            logESEParsingError(
                `Invalid or missing coordinates for circle sectorline: "${line}"`
            );
            return;
        }
        ...
        try {
            ...
            if (isNaN(radius)) {
                logESEParsingError(
                    `Invalid radius for circle sectorline: "${line}" - radius: "${parts[parts.length === 5 ? 4 : 3]}"`
                );
                return;
            }
            ...
            logESEParsingError(
                `Failed to create circle for sectorline: "${line}" - center: lat=${geo.lat}, lon=${geo.lon} - error: ${error}`
            );
        }
    }

    private static getCircleCenter(
        parts: string[],
        navaids: NseNavaid[]
        ...
        if (parts.length === 5) {
            ...
            if (!coords) {
                logESEParsingError(
                    `Failed to convert coordinates from parts: lat="${parts[2]}", lon="${parts[3]}"`
                );
                return null;
            }
        ...
        if (!navaid?.lat || !navaid?.lon) {
            logESEParsingError(
                `Navaid "${navaidName}" not found or missing coordinates`
            );
            return null;
        }
    ...
    private static handleCoord(
        ...
        if (geo) {
            ...
            logESEParsingError(
                `Failed to convert coordinates to cartesian: lat="${lat}", lon="${lon}"`
            );
        }
    }

    private static handleNewSector(
        line: string,
        result: ParsedEseContent,
        context: SectorHandlerContext
    ): void {
        ...
        if (isNaN(floorNum) || isNaN(ceilingNum)) {
            logESEParsingWarning(
                `Invalid altitude values for sector "${name}": floor="${floor}", ceiling="${ceiling}"`
            );
        }
    ...
    private static handleBorder(
        ...
        if (invalidBorders.length > 0) {
            logESEParsingWarning(
                `Sector "${context.currentSector.name}" references undefined border IDs: ${invalidBorders.join(", ")}`
            );
        }
    }

    private static handleDepApt(
        line: string,
        context: SectorHandlerContext
    ): void {
    ...
    private static handleActive(
        ...
        if (!icao || !runway) {
            logESEParsingWarning(
                `Invalid ACTIVE line format: "${line}" - expected ICAO and runway`
            );
            return;
        }
    ...
    private static handleDisplaySectorLine(
        line: string,
        context: SectorHandlerContext
    ): void {
        const [borderId, mySector, ...others] = this.splitAndClean(
            line,
            "DISPLAY_SECTORLINE"
        );
        ...
        if (isNaN(borderIdNum)) {
            logESEParsingError(
                `Invalid border ID in DISPLAY_SECTORLINE: "${borderId}" in line: "${line}"`
            );
            return;
        }
        ...
        context.currentSector.displaySectorLines.push({
            ...
            mySector,
            otherSectors: others
                .map((item) => item.replace(mySector, ""))
                .filter((item) => item !== ""),
        });
    }
}
