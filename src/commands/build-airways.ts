...
interface Subnetwork {
  level: Level;
  fixToIdx: Map<string, number>;
  fixes: { ident: string; lat: number; lon: number }[];
  segPairToIdx: Map<number, number>;
  segments: { fromIdx: number; toIdx: number; minLevel: number; canTraverse: boolean }[];
}
...
interface ParsedNetwork {
  isecCount: number;
  airwayLineCount: number;
  subnetCount: number;
  segmentCount: number;
  airwaysByName: Map<string, Subnetwork[]>;
  isecWaypoints: Map<string, { ident: string; lat: number; lon: number }>;
}

function parseIsec(buf: Buffer, onProgress: (n: number) => void): Map<string, { ident: string; lat: number; lon: number }> {
  const map = new Map<string, { ident: string; lat: number; lon: number }>();
  let count = 0;
  let lineStart = 0;
      ...
      if (t1 > -1 && t1 < lineEnd) {
        ...
        if (t2 > -1 && t2 < lineEnd) {
          ...
          if (ident && Number.isFinite(lat) && Number.isFinite(lon)) {
            ...
            if (!map.has(key)) map.set(key, { ident, lat, lon });
            count++;
            if ((count & 16383) === 0) onProgress(count);
          }
        }
      }
    }
...
function findOrCreateSubnet(list: Subnetwork[], level: Level, fromIdent: string, toIdent: string): Subnetwork {
  for (let i = 0; i < list.length; i++) {
    const s = list[i];
    if (s.level !== level) continue;
    if (s.fixToIdx.has(fromIdent) || s.fixToIdx.has(toIdent)) return s;
  }
  const fresh: Subnetwork = {
    level,
    fixToIdx: new Map(),
    fixes: [],
...
function addFixToSubnet(s: Subnetwork, ident: string, lat: number, lon: number): number {
...
function addSegment(s: Subnetwork, fromIdx: number, toIdx: number, minLevel: number, canTraverse: boolean): void {
...
export function parseAirwayFile(text: string, onProgress?: (n: number) => void): {
  airwaysByName: Map<string, Subnetwork[]>;
  lineCount: number;
} {
  const airwaysByName = new Map<string, Subnetwork[]>();
  let lineCount = 0;
  let lineStart = 0;
  ...
  for (let i = 0; i <= totalLen; i++) {
    ...
    for (let n = 0; n < 2; n++) {
      const base = n === 0 ? 6 : 11;
      if (fields.length < base + 5) break;
      const nIdent = fields[base];
      if (!nIdent || nIdent === "N") continue;
    ...
    lineCount++;
    if (onProgress && (lineCount & 8191) === 0) onProgress(lineCount);
    lineStart = i + 1;
  }
...
export function buildAirwaysDb(opts: BuildOptions): {
  waypoints: number;
  airways: number;
  segments: number;
} {
  const { airwayPath, isecPath, outputPath, silent } = opts;
  if (!fs.existsSync(airwayPath)) throw new Error(`airway.txt not found: ${airwayPath}`);
  if (!fs.existsSync(isecPath)) throw new Error(`isec.txt not found: ${isecPath}`);

  const startSpinner = (text: string): Ora | null => {
    if (silent) return null;
    return ora(text).start();
  };
  const setText = (s: Ora | null, t: string) => {
    if (s) s.text = t;
  };
  const succeed = (s: Ora | null, t: string) => {
    if (s) s.succeed(t);
  };

  const isecSpin = startSpinner("Reading isec.txt");
  const isecBuf = fs.readFileSync(isecPath);
  ...
  const airwayText = fs.readFileSync(airwayPath, "utf8");
  setText(airwaySpin, `Parsing airway.txt (${(airwayText.length / 1048576).toFixed(1)} MB)`);
  const { airwaysByName, lineCount } = parseAirwayFile(airwayText, (n) =>
    setText(airwaySpin, `Parsing airway.txt — ${n.toLocaleString()} lines`)
  );
  let subnetCount = 0;
  let segmentCount = 0;
  for (const list of airwaysByName.values()) {
    subnetCount += list.length;
    for (const s of list) segmentCount += s.segments.length;
  }
  succeed(
    airwaySpin,
    `Parsed ${lineCount.toLocaleString()} airway lines → ${subnetCount.toLocaleString()} airways, ${segmentCount.toLocaleString()} segments`
  );

  if (fs.existsSync(outputPath)) fs.unlinkSync(outputPath);
  const dbSpin = startSpinner(`Writing ${path.basename(outputPath)}`);
  ...
  const insertSegment = db.prepare(
    "INSERT INTO direct_segments (airway_id, from_waypoint_id, to_waypoint_id, minimum_level, can_traverse, from_identifier, to_identifier, airway_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  );

  const waypointIdCache = new Map<string, number>();
  const getOrInsertWaypoint = (ident: string, lat: number, lon: number): number => {
    const key = ident + "|" + lat + "|" + lon;
    ...
    if (info.changes > 0) {
      ...
      const row = selectWaypoint.get(ident, lat, lon) as { id: number } | undefined;
      if (!row) throw new Error(`Failed to resolve waypoint id for ${ident}`);
      id = row.id;
    }
  ...
  const tx = db.transaction(() => {
    ...
    for (const [name, list] of airwaysByName) {
      for (const subnet of list) {
        ...
        airwaysWritten++;
        const fixIds = new Array<number>(subnet.fixes.length);
  ...
  succeed(
    dbSpin,
    `Wrote ${outputPath} — ${airwaysWritten.toLocaleString()} airways, ${segmentsWritten.toLocaleString()} segments`
  );

  return { waypoints: waypointsWritten, airways: airwaysWritten, segments: segmentsWritten };
}

export async function buildAirwaysCommand(navDataPath: string, outputPath?: string): Promise<void> {
  const stat = fs.existsSync(navDataPath) ? fs.statSync(navDataPath) : null;
  let airwayPath: string;
  let isecPath: string;
  if (stat && stat.isDirectory()) {
    airwayPath = path.join(navDataPath, "airway.txt");
    isecPath = path.join(navDataPath, "isec.txt");
  } else {
  ...
  console.log(
    `Done in ${elapsed}s — ${result.waypoints.toLocaleString()} new waypoints, ${result.airways.toLocaleString()} airways, ${result.segments.toLocaleString()} segments`
  );
}
